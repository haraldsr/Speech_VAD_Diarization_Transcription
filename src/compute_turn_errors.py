...
def compute_turn_errors(
    ...
    ref_speakers_sorted = np.array(
        ["".join(sorted(str(s))) for s in df_ref["speaker"]], dtype=object
    )
    est_speakers_sorted = np.array(
        ["".join(sorted(str(s))) for s in df_est["speaker"]], dtype=object
    )
    ...
    ref_group_indices = {
        key: np.flatnonzero(ref_keys == key) for key in np.unique(ref_keys)
    }
    ...
    for key in np.unique(est_keys):
        ...
        intersection_duration = np.maximum(
            0.0,
            np.minimum(ref_end_block, est_end_block)
            - np.maximum(ref_start_block, est_start_block),
        )
        union_duration = np.maximum(ref_end_block, est_end_block) - np.minimum(
            ref_start_block, est_start_block
        )
    ...
    if isinstance(min_overlap_ratio, dict):
        type_thresholds = {
            str(key): float(value) for key, value in min_overlap_ratio.items()
        }
    ...
    if not np.all(matches_to_ref <= 1):
        _maybe_warn(
            "Multiple matches found for a single ground truth event.", suppress_warnings
        )
    ...
    if not np.all(matches_to_est <= 1):
        _maybe_warn(
            "Multiple matches found for a single estimated event.", suppress_warnings
        )
...
def tabulate_floor_transfers(
    ...
    if same_speaker_skips > 0:
        _maybe_print(
            f"Skipped {same_speaker_skips} same-speaker turn adjacencies when tabulating floor transfers.",
            suppress_prints,
        )

    # Create DataFrame from collected rows (more efficient and avoids FutureWarning)
    df_fto = (
        pd.DataFrame(floor_transfers)
        if floor_transfers
        else pd.DataFrame(
            columns=["speaker", "start_sec", "end_sec", "duration_sec", "type"]
        )
    )
...
def print_error_summary(err: dict, suppress_prints: bool = False) -> None:
    ...
    for type, metrics in err.items():
        ...
        _maybe_print(
            ...
            f"{metrics['duration_delta'][1]:.2f} seconds "
            f"[5%: {metrics['duration_delta'][0]:.2f}s, "
            f"95%: {metrics['duration_delta'][2]:.2f}s]",
            suppress_prints,
        )
        _maybe_print(
            f"{type} Mean Absolute Start Delta: "
            f"{metrics['start_delta'][1]:.2f} seconds "
            f"[5%: {metrics['start_delta'][0]:.2f}s, "
            f"95%: {metrics['start_delta'][2]:.2f}s]",
            suppress_prints,
        )
        _maybe_print(
            f"{type} Mean Absolute End Delta: "
            f"{metrics['end_delta'][1]:.2f} seconds "
            f"[5%: {metrics['end_delta'][0]:.2f}s, "
            f"95%: {metrics['end_delta'][2]:.2f}s]",
            suppress_prints,
        )
        _maybe_print(
            f"{type} Mean Absolute Start/End Delta: "
            f"{metrics['start_end'][1]:.2f} seconds "
            f"[5%: {metrics['start_end'][0]:.2f}s, "
            f"95%: {metrics['start_end'][2]:.2f}s]",
            suppress_prints,
        )
...
def compute_all_errors(
    ...
    for type in err_df["type"].unique():
        ...
        for delta_col in ["duration_delta", "start_delta", "end_delta"]:
            mean_abs = (
                detected_df[delta_col].abs().mean()
                if len(detected_df) > 0
                else float("nan")
            )
            p975_abs = (
                detected_df[delta_col].abs().quantile(0.975)
                if len(detected_df) > 0
                else float("nan")
            )
            p025_abs = (
                detected_df[delta_col].abs().quantile(0.025)
                if len(detected_df) > 0
                else float("nan")
            )
        ...
        mean_abs_start_end = (
            np.mean(
                np.concatenate(
                    [
                        detected_df["start_delta"].abs().values,
                        detected_df["end_delta"].abs().values,
                    ]
                )
            )
            if len(detected_df) > 0
        ...
        err[type]["start_end"] = np.array(
            [p025_abs_start_end, mean_abs_start_end, p975_abs_start_end]
        )
    return err, err_df
...
def validate_turns(
    ...
    _maybe_print(
        "Validating column names, labels, durations, and self-intersecting events.",
        suppress_prints,
    )
    _maybe_print("=" * 70, suppress_prints)
...
def check_labels(df: pd.DataFrame, suppress_prints: bool = False) -> pd.DataFrame:
    ...
    if not set(df["speaker"].unique()).issubset(allowed_speaker_labels):
        raise ValueError(
            "Speaker labels could not be replaced. "
            "Remaining invalid labels: "
            f"{set(df['speaker'].unique()) - allowed_speaker_labels}."
        )
    elif not set(df["type"].unique()).issubset(allowed_type_labels):
        raise ValueError(
            "Type labels could not be replaced. "
            "Remaining invalid labels: "
            f"{set(df['type'].unique()) - allowed_type_labels}."
        )
    else:
...
def check_durations(df: pd.DataFrame, suppress_prints: bool = False) -> pd.DataFrame:
    ...
    if np.any(msk_mis):
        _maybe_print(
            "Duration inconsistencies found (end_sec-start_sec does not match duration_sec):",
            suppress_prints,
        )
        _maybe_print(df[msk_mis].to_string(), suppress_prints)
        _maybe_print(
            "Replacing duration_sec with end_sec-start_sec for inconsistent entries.",
            suppress_prints,
        )
        df.loc[msk_mis, "duration_sec"] = (
            df.loc[msk_mis, "end_sec"] - df.loc[msk_mis, "start_sec"]
        )
    else:
...
def check_self_intersection(
    df: pd.DataFrame, suppress_prints: bool = False
) -> pd.DataFrame:
    _maybe_print(
    ...
    for i in df.index:
        ...
        msk_previous_selfoverlap = find_intersecting_events(
            df,
        ...
        if np.any(msk_previous_selfoverlap):
            df.loc[i, "self_intersect"] = "_".join(
                map(str, df.index[msk_previous_selfoverlap].tolist())
            )
    ...
    if np.any(df["self_intersect"] != ""):
        ...
        _maybe_print(
            "Keeping only first entry and removing self-intersecting entries for each speaker.",
            suppress_prints,
        )
...
def find_embedding_events(
    df: pd.DataFrame, t_start: float, t_end: float, mask: pd.Series[bool] | None = None
) -> pd.Series[bool]:
...
def merge_events(df: pd.DataFrame, idx1: int, idx2: int) -> pd.DataFrame:
    """Merge two events by updating first's end time and duration.

    Marks the second event for deletion.
    """
    ...
    if "transcription" in df.columns:
        df.loc[idx1, "transcription"] = (
            f"{df.loc[idx1, 'transcription']} {df.loc[idx2, 'transcription']}"
        )
    return df


def find_intersecting_events(
    df: pd.DataFrame, t1: float, t2: float, mask: pd.Series[bool] | None = None
) -> pd.Series[bool]:
    ...
    intersection = np.maximum(
        0.0, np.minimum(ends, t_end) - np.maximum(starts, t_start)
    )
...
def plot_turns(
    *dfs: pd.DataFrame,
    ...
    if titles is None:
        titles = [
            f"Turns Plot {i+1}" if len(dfs) > 1 else "Turns Plot"
            for i in range(len(dfs))
        ]
    elif len(titles) != len(dfs):
        raise ValueError(
            f"Number of titles ({len(titles)}) must match number of DataFrames ({len(dfs)})"
        )
            ...
            for turn_type in sorted(turn_types):
                ...
                for _, turn in type_df.iterrows():
                    jitter = np.random.uniform(
                        -0.1, 0.1
                    )  # Add jitter to show self-intersections
                    ax.plot(
...
def postprocess_turn_df(
    ...
    def _merge_same_speaker_segments(
        ...
        while i < len(idx_segments) - 1:
            ...
            while j < len(idx_segments):
                ...
                blocker_active = blocker_mask & _active_rows_mask(frame)
                is_blocked = np.any(
    ...
    def _merge_bridge_backchannels_into_turns(
        ...
        for bc_idx in idx_bc:
            ...
            prev_turn_candidates = [
                idx for idx in idx_turn if frame.loc[idx, "end_sec"] <= bc_start
            ]
            next_turn_candidates = [
                idx for idx in idx_turn if frame.loc[idx, "start_sec"] >= bc_end
            ]
    ...
    _maybe_print(
        f">>> Starting postprocessing - recursion iteration {rec_iter}", suppress_prints
    )
    turns_merged = 0

    # 1. Merge consecutive turns by the same speaker if they are not
    # separated by any interlocutor turn or self-backchannel
    for speaker in speakers:
        msk_active = _active_rows_mask(df)
        msk_int = (df["speaker"] == speakers[1 - speakers.index(speaker)]) & msk_active
        msk_turn = df["type"] == "turn"
        msk_spk_bc = (
            (df["speaker"] == speaker) & msk_active & (df["type"] == "backchannel")
        )
    ...
    for speaker in speakers:
        ...
        df, merged_bridge_count = _merge_bridge_backchannels_into_turns(
            df, speaker=speaker
        )
    ...
    # are valid and should not trigger endless recursion.
    recur = False
    for speaker in speakers:
        active_mask = _active_rows_mask(df)
        msk_spk_turn = (df["speaker"] == speaker) & active_mask & (df["type"] == "turn")
        idx_turns = df[msk_spk_turn].sort_values(by="start_sec").index.tolist()
        if len(idx_turns) < 2:
            continue

        msk_int_turn = (df["speaker"] != speaker) & active_mask & (df["type"] == "turn")
        msk_spk_bc = (
            (df["speaker"] == speaker) & active_mask & (df["type"] == "backchannel")
        )
        blocker_mask = msk_int_turn | msk_spk_bc

        for i in range(len(idx_turns) - 1):
            idx_left = idx_turns[i]
            idx_right = idx_turns[i + 1]
            blocked = np.any(
                find_intersecting_events(
                    df,
                    t1=df.loc[idx_left, "end_sec"],
                    t2=df.loc[idx_right, "start_sec"],
                    mask=blocker_mask,
                )
            )
            if not blocked:
                recur = True
                _maybe_warn(
                    "Consecutive turns by the same speaker found at "
                    f"index {idx_left} and {idx_right} after merging "
                    "with no blocker. Recursing for another iteration.",
                    suppress_warnings,
                )
                _maybe_print(
                    "Consecutive turns by the same speaker found at "
    ...
    if rec_iter < max_iter:
        if recur:
            ...
            _maybe_print(
                "No consecutive turns found. Post-processing complete.", suppress_prints
            )
            _maybe_print("-" * 60, suppress_prints)
    else:
        _maybe_warn(
...
if __name__ == "__main__":
    ...
    dfp_est.to_csv(
        "outputs/demo/demo_labels_proc.txt", sep="\t", index=True, index_label="index"
    )
    ...
    plot_turns(
        ...
        dfp_est,
        titles=[
            "Original turn data - estimate (demo)",
            "After validation",
    ...
    dfp_ref.to_csv(
        "outputs/demo/F1F2_quiet_food_1m_01_labels_manual_rinor_proc.txt",
        sep="\t",
        index=True,
        index_label="index",
    )
    ...
    plot_turns(
        ...
        dfp_ref,
        titles=[
            "Original turn data - reference (demo)",
            "After validation",
            "After post-processing",
        ],
    )
